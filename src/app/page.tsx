...
export default function Home() {
    const { isSidebarOpen, setSidebarOpen, currentView, setCurrentView } = useSettingsStore();
    const [isFormOpen, setIsFormOpen] = useState(false);
    const [showSettings, setShowSettings] = useState(false);
    const [formMode, setFormMode] = useState<'add' | 'edit'>('add');
    const [selectedEvent, setSelectedEvent] = useState<CalendarItem | undefined>(undefined);
    const [searchQuery, setSearchQuery] = useState('');
    const eventCount = useLiveQuery(() => db.events.count()); // For testing
    ...
    <div className="flex h-screen overflow-hidden bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100">
      ...
      <aside className={`
        ...
        fixed inset-y-0 left-0 z-50 w-72 bg-slate-50 dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800
        ...
        <div className="p-4 flex justify-between items-center border-b border-slate-200 dark:border-slate-800">
          ...
          <button onClick={() => setSidebarOpen(false)} className="p-2 hover:bg-slate-200 cursor-pointer dark:hover:bg-slate-800 rounded">
            X
          </button>
        </div>
          ...
          {['Month', 'Day', 'Year', 'Schedule', 'Settings'].map((view) => (
            <button
              ...
              className={`w-full text-left px-4 py-2 rounded-lg transition-colors ${
                currentView === view.toLowerCase() ? 'bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-200 font-bold' : 'hover:bg-slate-200 cursor-pointer dark:hover:bg-slate-800'
              }`}
            >
              {view}
            </button>
