...
export default function YearView() {
  ...
  // Fetch and Expand events for the entire year
  ...
  const buildMonthAriaLabel = (monthDate: Date) => {
    ...
    const monthEvents = (yearEvents || []).filter(
      e => e.startMs <= monthEnd && e.endMs >= monthStart
    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {months.map((monthDate) => (
            <button 
              ...
              className={`p-4 rounded-2xl shadow-sm border hover:cursor-pointer group flex flex-col items-stretch
